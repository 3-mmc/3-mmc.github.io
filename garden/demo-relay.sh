...
DATA="garden/data/olive.json"
INTERVAL="${INTERVAL:-600}"
HOURS="${HOURS:-6}"
DEVICE="${OLIVE_DEVICE:-http://192.168.1.205}"
...
echo "relay: page at https://3-mmc.github.io/garden/"
...
while [ "$(date +%s)" -lt "$deadline" ]; do
  ...
  if ! python3 garden/relay.py --device "$DEVICE" --out "$DATA" >/dev/null 2>&1; then
    ...
    git add -- "$DATA"
    git commit -q -m "Update olive snapshot" -- "$DATA"
    if git push -q origin HEAD 2>/dev/null; then
      pushed=$(( pushed + 1 ))
      echo "$(date '+%H:%M:%S')  [$n] pushed  $(python3 -c "
import json; d=json.load(open('$DATA'))['latest']
p=d.get('soil_pct')
print((f\"{p:.0f}% soil \" if isinstance(p,(int,float)) else 'soil n/a ') + f\"{d.get('temp_c',float('nan')):.1f}C\")" 2>/dev/null)"
    else
...
echo "relay: the page now shows the last copy, ageing as a committed snapshot"
